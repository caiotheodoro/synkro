          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-2">
            {section.fields.map((field, fieldIndex) => (
              <div key={fieldIndex} className="mb-2">
                <div className="text-sm font-medium text-black uppercase">
                  {field.label}
                </div>
        ...
        <div className="mt-4">
          {config.relatedEntities.map((relatedEntity, index) => {
            ...
            return (
              <div
                key={index}
                className="p-6 bg-white border-4 border-black rounded-lg shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] mb-6"
              >
                <h3 className="mb-4 text-xl font-bold border-b-4 border-black pb-2">
                  {relatedEntity.title}
                </h3>
                ...
                {entityData.length === 0 ? (
                  <p className="text-neutral-500">
                    No {relatedEntity.title.toLowerCase()} found
                  </p>
                ) : (
