      ...
      if (module.detailConfig?.relatedEntities) {
        ...
        for (const relatedEntity of module.detailConfig.relatedEntities) {
          try {
            ...
            const response = await fetch(`/api/${relatedEntity.entity}?${relatedEntity.relationField}=${id}`);
            const data = await response.json();
            relatedData[relatedEntity.entity] = data.data || data;
          } catch (err) {
            console.error(`Error fetching related entity ${relatedEntity.entity}:`, err);
          }
        }
