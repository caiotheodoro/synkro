...
export const DataTable = <T extends Record<string, any>>({
  ...
  return (
    <div
      className={cn(
        "relative border-3 border-black rounded-lg bg-white w-full overflow-hidden",
        className
      )}
    >
        ...
        <table className="w-full border-collapse">
          <thead className="bg-gray-50 border-b-3 border-black">
            <tr>
              ...
              {hasActions && (
                <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider sticky right-0 bg-gray-50 z-10 min-w-[150px]">
                  Actions
                </th>
              )}
            </tr>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {data.map((item, index) => (
              <tr key={index} className="hover:bg-gray-50">
                ...
                {hasActions && (
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium sticky right-0 bg-white z-10 min-w-[150px] shadow-[-4px_0_4px_rgba(0,0,0,0.05)]">
